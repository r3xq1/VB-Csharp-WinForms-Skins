...
Module DrawHelpers
    Public Function RoundRectangle(Rectangle As Rectangle, Curve As Integer) As GraphicsPath
...
#Region "Declarations"
    Private State As MouseState = MouseState.None
    Private _FontColour As Color = Color.White ' Изменено на белый для лучшей видимости
    Private _BaseColour As Color = Color.FromArgb(45, 45, 48)
    ...
    Private _ShowText As Boolean = True ' Убедитесь, что значение по умолчанию true
            ...
            If _ShowImage Then
                If (Width > 50) AndAlso (Height > 30) Then
                    If _ImageAlignment = __ImageAlignment.Left Then
                        ...
                        If _ShowText Then
                            .DrawString(Text, Font, New SolidBrush(_FontColour), New Rectangle(10 + Height - 20 + 10, 0, Width - 20 - (Height - 10) - 10, Height), New StringFormat With {.Alignment = _TextAlignment, .LineAlignment = StringAlignment.Center})
                        End If
                    ElseIf _ImageAlignment = __ImageAlignment.Right Then
                        .DrawImage(_Image, New Rectangle((Width - Height + 10), 10, Height - 20, Height - 20))
                        If _ShowText Then
                            .DrawString(Text, Font, New SolidBrush(_FontColour), New Rectangle(10, 0, Width - 20 - (Height - 20) - 10, Height), New StringFormat With {.Alignment = _TextAlignment, .LineAlignment = StringAlignment.Center})
                        End If
                    End If
                End If
            Else
...
#Region "Properties"
    ...
    Public Sub AddItem(Items As String)
        Dim Item As New VSListBoxItem With {
            .Text = Items
        }
        _Items.Add(Item)
        Invalidate()
    ...
    Public Sub RemoveItem(item As VSListBoxItem)
        _Items.Remove(item)
        Invalidate()
        InvalidateScroll()
    End Sub

    Public Sub RemoveItems(items As VSListBoxItem())
    ...
    Private Sub Vertical_MouseDown(sender As Object, e As MouseEventArgs)
        Focus()
    End Sub
...
#Region "Draw Control"
    ...
    Sub New()
        SetStyle(ControlStyles.OptimizedDoubleBuffer Or ControlStyles.AllPaintingInWmPaint Or ControlStyles.ResizeRedraw Or
                    ...
                    ControlStyles.SupportsTransparentBackColor, True)
        DoubleBuffered = True
        VerticalScrollbar = New VisualStudioVerticalScrollBar With {
            ._SmallChange = 1,
            ._LargeChange = 1
        }
