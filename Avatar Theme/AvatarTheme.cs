...
class AvatarTheme : ThemeContainer154
{
    private Pen Border;
    private Brush TextBrush;
    public bool ShowIcon { get; set; } = true; // Свойство для управления отображением иконки

    public AvatarTheme()
    {
        TabStop = false;
        TransparencyKey = Color.Transparent; // Изменено с Fuchsia на Transparent
        BackColor = Color.Black;
        Font = new Font("Century Gothic", 9);
        SetColor("Text", Color.DeepSkyBlue);
        SetColor("Border", Color.Black); // Изменен цвет границы на черный
    }
    ...
    {
        using (var LGB1 = new LinearGradientBrush(new Rectangle(0, 0, Width, Height),
        ...
        if (form != null)
        {
            G.DrawString(form.Text, Font, TextBrush, new Point(25, 5));
            if (ShowIcon) // Проверка свойства ShowIcon
            {
                G.DrawIcon(form.Icon, new Rectangle(7, 6, 16, 16));
            }
        }

        using (var LGB2 = new LinearGradientBrush(new Rectangle(0, 0, Width, Height),
                                                  Color.FromArgb(125, Color.DeepSkyBlue),
                                                  Color.FromArgb(175, 25, 25, 112),
                                                  85.0F))
        {
...
{
    private Color buttonColorTop, buttonColorBottom;
    ...
    protected override void PaintHook()
    {
        ...
        switch (State)
        {
            ...
            case MouseState.Over:
                using (var overlayBrush = new SolidBrush(Color.FromArgb(125, Color.Black)))
                {
            ...
            case MouseState.Down:
                using (var overlayBrush = new SolidBrush(Color.FromArgb(200, Color.Black)))
                {
                    G.FillRectangle(overlayBrush, new Rectangle(0, 0, Width, Height));
                }
                DrawText(textColor, HorizontalAlignment.Center, 0, 0);
...
public class AvatarGroupBox : ThemeContainer154
{
    ...
    protected override void PaintHook()
    {
        ...
        using (var backBg = new LinearGradientBrush(
        ...
        using (var topBg = new LinearGradientBrush(
...
public class DroneSeperator : ThemeControl154
{
    ...
    public Orientation Orientation
    {
        ...
        {
            _orientation = value;
            LockHeight = value == Orientation.Vertical ? 0 : 14;
            LockWidth = value == Orientation.Vertical ? 14 : 0;
            Invalidate();
        }
    }
    ...
    protected override void PaintHook()
    {
        ...
        var bl1 = new ColorBlend
        {
            Positions = new float[] { 0.0f, 0.15f, 0.85f, 1.0f },
            Colors = new Color[] { Color.Transparent, Color.Black, Color.Black, Color.Transparent }
        };

        var bl2 = new ColorBlend
        {
            Positions = new float[] { 0.0f, 0.15f, 0.5f, 0.85f, 1.0f },
            Colors = new Color[] { Color.Transparent, Color.FromArgb(35, 35, 35), Color.FromArgb(45, 45, 45), Color.FromArgb(35, 35, 35), Color.Transparent }
        };
...
public class DroneProgressBar : ThemeControl154
{
    ...
    public DroneProgressBar()
    {
        ...
        {
            Colors = new Color[] { Color.FromArgb(0, 55, 90), Color.FromArgb(0, 66, 108), Color.FromArgb(0, 66, 108), Color.FromArgb(0, 55, 90) },
    ...
    protected override void OnCreation()
    {
        if (!DesignMode)
        {
            var thread = new Thread(MoveGlow) { IsBackground = true };
            thread.Start();
        }
    }
    ...
    private void MoveGlow()
    {
        ...
        {
            glowPosition += 0.01f;
            if (glowPosition >= 1.0f) glowPosition = -1.0f;
            Invalidate();
            Thread.Sleep(25);
        }
    }
    ...
    protected override void PaintHook()
    {
        ...
        int progress = (int)((_value / (float)_maximum) * Width);
        if (progress != 0)
        {
            ...
            DrawGradient(blend, (int)(glowPosition * progress), 0, progress, Height, 0.0f);
