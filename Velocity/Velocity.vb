...
Public Module Helpers
    ...
    Public Function b64Image(b64 As String) As Image
...
Public Class VelocityTitle
    ...
    Protected Overrides Sub OnPaint(e As PaintEventArgs)
        ...
        Dim txtSize As SizeF = g.MeasureString(Text, Font)
        Dim x As Single = 0
        Dim y As Single = (Height / 2) - (txtSize.Height / 2)

        Select Case _txtAlign
            Case TxtAlign.Left
                g.FillRectangle(New SolidBrush(BackColor), New Rectangle(18, (Height / 2) - CInt(txtSize.Height) - 2, CInt(txtSize.Width) + 6, CInt(txtSize.Height) + 6))
                x = 20

            Case TxtAlign.Center
                g.FillRectangle(New SolidBrush(BackColor), New Rectangle((Width / 2) - (CInt(txtSize.Width) / 2) - 2, (Height / 2) - CInt(txtSize.Height) / 2 - 2, CInt(txtSize.Width) + 2, CInt(txtSize.Height) + 2))
                x = (Width / 2) - (CInt(txtSize.Width) / 2)

            Case TxtAlign.Right
                g.FillRectangle(New SolidBrush(BackColor), New Rectangle(Width - (CInt(txtSize.Width) + 18), (Height / 2) - CInt(txtSize.Height) - 2, CInt(txtSize.Width) + 4, Height + 6))
                x = Width - (CInt(txtSize.Width) + 16)
        End Select

        ' Проверка на возможность преобразования текста в число
        Dim renderedText As String
        If IsNumeric(Text) Then
            renderedText = CInt(Text).ToString()
        Else
            renderedText = Text
        End If
...
Public Class VelocityProgressBar
    ...
    Protected Overrides Sub OnPaint(e As PaintEventArgs)
        ...
        If _showPercent Then
            ...
            g.DrawString(CInt(_val & "%"), Font, New SolidBrush(ForeColor), Width - 30, (Height / 2) - (g.MeasureString(_val & "%", Font).Height / 2) - 1)
        Else
