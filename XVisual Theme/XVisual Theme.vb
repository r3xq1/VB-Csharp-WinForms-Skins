...
Module Draw
    Public Function GetBrush(c As Color) As SolidBrush
        Return New SolidBrush(c)
    End Function
    Public Function GetPen(c As Color) As Pen
    ...
    Public Sub InnerGlow(G As Graphics, Rectangle As Rectangle, Colors As Color())
        ...
        Next
    End Sub
    Public Sub InnerGlowRounded(G As Graphics, Rectangle As Rectangle, Degree As Integer, Colors As Color())
        Dim SubtractTwo As Integer = 1
        Dim AddOne As Integer = 0
        For Each c In Colors
...
#Region " Control Help - Properties & Flicker Control "
    ...
    Public Overrides ReadOnly Property DisplayRectangle As System.Drawing.Rectangle
        ...
        End Get
    End Property
    Sub ReplaceItem(sender As System.Object, e As System.Windows.Forms.DrawItemEventArgs) Handles Me.DrawItem
        e.DrawBackground()
        Try
...
Public Class xVisualTabControl
    Inherits TabControl
    ...
    Protected Overrides Sub OnPaint(e As PaintEventArgs)
        MyBase.OnPaint(e) ' Вызов метода родительского класса
        Using B As New Bitmap(Width, Height)
            Using G As Graphics = Graphics.FromImage(B)
                ...
                If TabCount > 0 Then
                    ...
                    ' Фон вкладок
                    Dim tabBgRect As New Rectangle(0, 0, ItemSize.Height + 3, Height - 1)
                    ...
                    For i = 0 To TabCount - 1
                        ...
                        If i = SelectedIndex Then
                            Dim selectedTabRect As New Rectangle(tabRect.X - 2, tabRect.Y - 2, tabRect.Width + 3, tabRect.Height - 1)
                            Dim TabOverlay As New LinearGradientBrush(selectedTabRect, Color.FromArgb(114, 203, 232), Color.FromArgb(58, 118, 188), 90S)
                            G.FillRectangle(TabOverlay, selectedTabRect)
                            G.DrawString(TabPages(i).Text, FF, Brushes.White, selectedTabRect, New StringFormat With {.LineAlignment = StringAlignment.Center, .Alignment = StringAlignment.Center})
                        Else
                            G.DrawString(TabPages(i).Text, FF, Brushes.Gray, tabRect, New StringFormat With {.LineAlignment = StringAlignment.Center, .Alignment = StringAlignment.Center})
                        End If
                    Next
                End If
    ...
    Function GetPen(color As Color) As Pen
        Return New Pen(color)
    End Function

    Function GetBrush(color As Color) As Brush
    ...
    ' Другая необходимая обработка, если есть функция NoiseBrush
    Private Function NoiseBrush(colors As Color()) As TextureBrush
        ' Реализация этого метода должна быть добавлена
        ' Например, создаёт текстуру с шумом из переданных цветов
        Return New TextureBrush(New Bitmap(1, 1)) ' Временная заглушка
    End Function
End Class
