...
public abstract class ThemeContainer154 : ContainerControl
{
    ...
    // Новый метод для установки толщины границы
    protected void DrawBorders(Pen pl, int thickness)
    {
        Rectangle borderRectangle = new Rectangle(0, 0, Width, Height);
        DrawBorders(pl, borderRectangle.X, borderRectangle.Y, borderRectangle.Width, borderRectangle.Height, thickness);
    }
...
public class VitalityTheme : UserControl
{
    private Color G1, G2, BG;
    ...
    public VitalityTheme()
    {
        ...
        Dock = DockStyle.Fill; // Установка Dock на Fill по умолчанию
    }

    private void InitializeComponent()
    {
        DoubleBuffered = true; // Улучшение сглаживания
        Paint += VitalityTheme_Paint; // Подписка на событие рисования
    }

    protected virtual void SetColor(string name, Color color)
    {
        switch (name)
        {
            case "G1": G1 = color; break;
            case "G2": G2 = color; break;
            case "BG": BG = color; break;
        }
    }

    protected virtual Color GetColor(string name)
    {
        return name switch
        {
            "G1" => G1,
    ...
    private void VitalityTheme_Paint(object sender, PaintEventArgs e)
    {
        PaintHook(e.Graphics);
    }

    protected virtual void PaintHook(Graphics G)
    {
        if (Width <= 0 || Height <= 0) return; // Избегаем краша

        G.Clear(BG);
        DrawGradientBackground(G);
        DrawSeparatorLines(G);
        DrawBorders(G, Pens.LightGray, 1);
        DrawIconAndTitle(G);
    }

    private void DrawGradientBackground(Graphics G)
    {
        using (var brush = new LinearGradientBrush(new Rectangle(1, 1, Width - 2, 23), G1, G2, 90.0F))
        {
            G.FillRectangle(brush, new Rectangle(1, 1, Width - 2, 23));
        }
    }
    ...
    private void DrawIconAndTitle(Graphics G)
    {
        if (Parent?.FindForm()?.Icon != null)
        {
            var iconRect = new Rectangle(3, 3, 20, 20);
            G.DrawIcon(Parent.FindForm().Icon, iconRect);
        }

        if (!string.IsNullOrEmpty(Parent?.FindForm().Text))
        {
            using (var font = new Font("Segoe UI", 9))
            {
                G.DrawString(Parent.FindForm().Text, font, Brushes.Gray, new Point(25, 5));
            }
        }
    }

    protected void DrawBorders(Graphics G, Pen pen, int thickness)
    {
        G.DrawRectangle(pen, 0, 0, Width - thickness, Height - thickness);
    }
}
    ...
    protected override void PaintHook()
    {
        ...
        switch (State)
        {
            case MouseState.Over:
                fillColor1 = G1; // Используем G1 для состояния "приподнят"
                fillColor2 = G2; // Используем G2 для градиента
                break;
            case MouseState.Down:
                fillColor1 = G2; // Используем G2 для состояния "нажато"
                fillColor2 = Color.White; // Используем белый для градиента
                break;
            case MouseState.None:
            default:
                fillColor1 = G1; // Используем G1 для состояния "нормальное"
                fillColor2 = G2; // Используем G2 для градиента
                break;
        }

        using (var lgb = new LinearGradientBrush(new Rectangle(Point.Empty, new Size(Width, Height)), fillColor1, fillColor2, 90.0F))
        {
            G.FillRectangle(lgb, new Rectangle(Point.Empty, new Size(Width, Height)));
        }
...
public class VitalityProgressbar : ThemeControl154
{
    ...
    protected override void PaintHook()
    {
        ...
        using (var lgb = new LinearGradientBrush(new Rectangle(new Point(2, 2), new Size(Width - 2, Height - 5)), Color.White, BG, 90.0F))
        {
        ...
        using (var hb = new HatchBrush(HatchStyle.BackwardDiagonal, Color.LightGray, Color.Transparent))
        {
...
public class VitalityLabel : Label
{
...
public class VitalityTabControl : TabControl
{
    ...
    protected override void OnPaint(PaintEventArgs e)
    {
        using (Bitmap B = new Bitmap(Width, Height))
        {
            ...
            {
                try { SelectedTab.BackColor = Color.FromArgb(240, 240, 240); } catch { }
                ...
                for (int i = 0; i < TabCount; i++)
                {
                    if (i != SelectedIndex)
                    {
                        ...
                        using (LinearGradientBrush G1 = new LinearGradientBrush(new Point(x2.X, x2.Y), new Point(x2.X, x2.Y + x2.Height), Color.White, Color.LightGray))
                        {
                            ...
                            G.DrawString(TabPages[i].Text, Font, Brushes.Gray, x2, new StringFormat { LineAlignment = StringAlignment.Near, Alignment = StringAlignment.Center });
                        }
                    }
                }
                ...
                if (SelectedIndex != -1)
                {
                    ...
                    using (LinearGradientBrush GradientBrush = new LinearGradientBrush(new Rectangle(x1.X + 2, x1.Y + 2, x1.Width - 2, x1.Height), Color.White, Color.LightGray, 90.0F))
                    {
                        ...
                        G.DrawString(TabPages[SelectedIndex].Text, Font, Brushes.Gray, x1, new StringFormat { LineAlignment = StringAlignment.Center, Alignment = StringAlignment.Center });
                    }
                }
...
public class VitalityRadiobutton : ThemeControl154
{
    ...
    private void InvalidateControls()
    {
        ...
        {
            if (control != this && control is VitalityRadiobutton radioButton)
            {
