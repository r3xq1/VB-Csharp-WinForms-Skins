...
//public class ChromeThemeContainer : ThemeContainer154
//{
//    private Color TitleColor; // Цвет заголовка
//    private Color Xcolor;     // Цвет кнопки закрытия
//    private Color Xellipse;   // Цвет эллипса для кнопки закрытия
//    private int X, Y;         // Координаты мыши для отслеживания положения курсора

//    // Конструктор класса
//    public ChromeThemeContainer()
//    {
//        Dock = DockStyle.Fill;
//        TransparencyKey = Color.Fuchsia; // Устанавливаем ключ прозрачности
//        BackColor = Color.White;          // Устанавливаем фон
//        Font = new Font("Segoe UI", 9);   // Устанавливаем шрифт
//        SetColor("Title color", Color.Black); // Устанавливаем цвет заголовка
//        SetColor("X-color", 90, 90, 90); // Устанавливаем цвет кнопки закрытия
//        SetColor("X-ellipse", 114, 114, 114); // Устанавливаем цвет эллипса
//    }

//    // Метод для получения цветовых значений
//    protected override void ColorHook()
//    {
//        TitleColor = GetColor("Title color"); // Получаем цвет заголовка
//        Xcolor = GetColor("X-color");         // Получаем цвет кнопки закрытия
//        Xellipse = GetColor("X-ellipse");     // Получаем цвет эллипса
//    }

//    // Метод обработки перемещения мыши
//    protected override void OnMouseMove(MouseEventArgs e)
//    {
//        X = e.Location.X; // Сохраняем координату X курсора
//        Y = e.Location.Y; // Сохраняем координату Y курсора
...
//    {
//        // Проверка, попадает ли курсор в зону кнопки закрытия
//        if (new Rectangle(Width - 22, 5, 15, 15).Contains(new Point(X, Y)))
//        {
//            FindForm().Close(); // Закрываем форму, если кнопка была нажата
//        }
//        base.OnMouseClick(e); // Вызываем базовый метод
//    }

//    // Метод отрисовки пользовательского оформления
//    protected override void PaintHook()
//    {
//        G.Clear(BackColor); // Очищаем графику, заполняя фоном

//        // Рисуем углы с выбранным цветом
...
//        G.SmoothingMode = SmoothingMode.HighQuality; // Устанавливаем режим сглаживания

//        // Проверяем, находится ли курсор над кнопкой закрытия
//        if (new Rectangle(Width - 22, 5, 15, 15).Contains(new Point(X, Y)))
//        {
//            G.FillEllipse(new SolidBrush(Xellipse), new Rectangle(Width - 24, 6, 16, 16)); // Заполняем эллипс цветом Xellipse
//            G.DrawString("r", new Font("Webdings", 8), new SolidBrush(BackColor), new Point(Width - 23, 5)); // Рисуем значок
//        }
//        else
//        {
//            G.DrawString("r", new Font("Webdings", 8), new SolidBrush(Xcolor), new Point(Width - 23, 5)); // Рисуем значок с цветом Xcolor
//        }

//        DrawText(new SolidBrush(TitleColor), new Point(8, 7)); // Отображаем текст с цветом TitleColor
//    }
//}
...
public class ChromeThemeContainer : ThemeContainer154
{
    ...
    public ChromeThemeContainer()
    {
        Dock = DockStyle.Fill; // Устанавливаем Dock для заполнения всей формы
        TransparencyKey = Color.Fuchsia; // Устанавливаем ключ прозрачности
        BackColor = Color.WhiteSmoke; // Устанавливаем фон компонента
        this.BackColor = Color.WhiteSmoke; // Устанавливаем фон компонента
        Font = new Font("Segoe UI", 9); // Устанавливаем шрифт
        SetColor("Title color", Color.Black); // Устанавливаем цвет заголовка
        SetColor("X-color", 90, 90, 90); // Устанавливаем цвет кнопки закрытия
        SetColor("X-ellipse", 114, 114, 114); // Устанавливаем цвет эллипса
...
public class ChromeButton : ThemeControl154
{
    ...
    public ChromeButton()
    {
        ...
        Font = new Font("Segoe UI", 9); // Установка шрифта
    ...
    {
        G.Clear(BackColor); // Очищаем графику, заполняя фоном
        LinearGradientBrush LGB; // Градиентная кисть
        G.SmoothingMode = SmoothingMode.HighQuality; // Установка качества сглаживания

        // Выбор градиента в зависимости от состояния кнопки
        switch (State)
        {
        ...
        if (!Enabled)
        {
            LGB = new LinearGradientBrush(new Rectangle(0, 0, Width - 1, Height - 1), GTN, GBN, 90.0F); // Задний план для неактивной кнопки
        }

        GraphicsPath buttonPath = CreateRound(Rectangle.Round(LGB.Rectangle), 3); // Создание закругленного пути
        G.FillPath(LGB, buttonPath); // Заполнение кнопки
        if (!Enabled)
            G.FillPath(new SolidBrush(Color.FromArgb(50, Color.White)), buttonPath); // Полупрозрачный эффект для неактивной кнопки

        G.SetClip(buttonPath); // Установка обрезки по пути
        LGB = new LinearGradientBrush(new Rectangle(0, 0, Width, Height / 6), Color.FromArgb(80, Color.White), Color.Transparent, 90.0F);
        G.FillRectangle(LGB, Rectangle.Round(LGB.Rectangle)); // Заполнение верхней части кнопки

        G.ResetClip(); // Сброс обрезки
        G.DrawPath(new Pen(Bo), buttonPath); // Отрисовка границы кнопки

        // Отрисовка текста в зависимости от состояния кнопки
        if (Enabled)
        {
            switch (State)
            {
                case MouseState.None:
                    DrawText(new SolidBrush(TN), HorizontalAlignment.Center, 1, 0); // Обычный текст
                    break;
                default:
                    DrawText(new SolidBrush(TDO), HorizontalAlignment.Center, 1, 0); // Текст при наведении
                    break;
            }
        }
        else
        {
            DrawText(new SolidBrush(TD), HorizontalAlignment.Center, 1, 0); // Текст для неактивной кнопки
        }
    }
}
//public class ChromeCheckbox : ThemeControl154
//{
//    private int X; // Позиция мыши по оси X
//    private Color GTN, GTO, GTD, GBN, GBO, GBD, Bo, T; // Цвета
//    private bool _Checked; // Состояние чекбокса

//    // Конструктор класса
//    public ChromeCheckbox()
//    {
//        LockHeight = 17; // Фиксированная высота
//        Font = new Font("Segoe UI", 9); // Установка шрифта
//        SetColor("Gradient top normal", 237, 237, 237);
//        SetColor("Gradient top over", 242, 242, 242);
//        SetColor("Gradient top down", 235, 235, 235);
...
//    {
//        base.OnMouseMove(e);
//        X = e.Location.X; // Сохраняем позицию мыши
...
//    protected override void PaintHook()
//    {
//        G.Clear(BackColor); // Очищаем графику
//        LinearGradientBrush LGB; // Градиентная кисть
//        G.SmoothingMode = SmoothingMode.HighQuality; // Установка режима сглаживания

//        // Выбор градиента в зависимости от состояния чекбокса
//        switch (State)
//        {
//            case MouseState.None:
...
//        GraphicsPath buttonPath = CreateRound(Rectangle.Round(LGB.Rectangle), 5); // Создание закругленного пути
//        G.FillPath(LGB, CreateRound(Rectangle.Round(LGB.Rectangle), 3)); // Заполнение чекбокса
//        G.SetClip(buttonPath); // Установка обрезки
//        LGB = new LinearGradientBrush(new Rectangle(0, 0, 14, 5), Color.FromArgb(150, Color.White), Color.Transparent, 90.0F);
//        G.FillRectangle(LGB, Rectangle.Round(LGB.Rectangle)); // Заполнение верхней части чекбокса
//        G.ResetClip(); // Сброс обрезки
//        G.DrawPath(new Pen(Bo), buttonPath); // Отрисовка рамки чекбокса

//        DrawText(new SolidBrush(T), 17, -2); // Отрисовка текста

//        // Отображение отметки, если чекбокс проверен
//        if (_Checked)
//        {
...
//            G.DrawImage(check, new Rectangle(2, 3, check.Width, check.Height)); // Отображение отметки
//        }
//    }

//    // Свойство для состояния чекбокса (проверен/не проверен)
//    public bool Checked
//    {
//        get { return _Checked; } // Получение состояния
//        set
//        {
//            _Checked = value; // Установка состояния
//            Invalidate(); // Обновление изображения
//        }
//    }
...
//    {
//        _Checked = !_Checked; // Изменение состояния
//        CheckedChanged?.DynamicInvoke(this); // Вызываем событие изменения состояния
//        base.OnMouseDown(e); // Вызов базового метода
//    }
...
public class ChromeCheckbox : ThemeControl154
{
    ...
    protected override void PaintHook()
    {
        ...
        GraphicsPath buttonPath = CreateRound(Rectangle.Round(LGB.Rectangle), 5); // Создание закругленного пути
        G.FillPath(LGB, buttonPath); // Заполнение чекбокса
        G.DrawPath(new Pen(Bo), buttonPath); // Отрисовка рамки чекбокса

        DrawText(new SolidBrush(T), 17, -2); // Отрисовка текста

        // Отрисовка галочки, если чекбокс проверен
        if (_Checked)
        {
            // Определение точки для рисования галочки
            using (Pen checkPen = new Pen(T, 2))
            {
                G.DrawLine(checkPen, 3, 8, 6, 11); // Рисуем часть галочки
                G.DrawLine(checkPen, 6, 11, 11, 4); // Рисуем другую часть галочки
            }
        }
    }
...
{
    private int X; // Позиция мыши по оси X
    private Color TextColor, G1, G2, Bo, Bb; // Цвета
    private bool _Checked; // Состояние радиокнопки
    private int _Field = 16; // Высота поля
    ...
    protected override void PaintHook()
    {
        ...
        // Рисуем круг радиокнопки
        using (var LGB = new LinearGradientBrush(new Rectangle(new Point(0, 0), new Size(14, 14)), G1, G2, 90.0F))
        {
        ...
        {
            using (var SB = new SolidBrush(Color.FromArgb(20, Color.Black)))
            {
                G.FillEllipse(SB, new Rectangle(new Point(0, 0), new Size(14, 14)));
            }
        }

        // Создание графического пути для обрезки
        using (var P = new GraphicsPath())
        {
            P.AddEllipse(new Rectangle(0, 0, 14, 14));
            G.SetClip(P);

            using (var LLGGBB = new LinearGradientBrush(new Rectangle(0, 0, 14, 5), Color.FromArgb(150, Color.White), Color.Transparent, 90.0F))
            {
        ...
        G.ResetClip(); // Сброс обрезки
        G.DrawEllipse(new Pen(Bo), new Rectangle(new Point(0, 0), new Size(14, 14))); // Рисуем границу радиокнопки

        // Заполнение "пульсирующей" части
        if (_Checked)
        {
            using (var LGB = new SolidBrush(Bb))
            {
                G.FillEllipse(LGB, new Rectangle(new Point(4, 4), new Size(6, 6))); // Заполнение внутренней части
            }
        }

        DrawText(new SolidBrush(TextColor), HorizontalAlignment.Left, 17, -2); // Отрисовка текста
    }

    // Свойство для высоты поля
    public int Field
    {
        get { return _Field; }
        set
        {
            if (value < 4) return; // Минимальное значение
            _Field = value; // Установка значения
            LockHeight = value; // Установка фиксированной высоты
            Invalidate(); // Обновление изображения
        }
    }

    // Свойство для состояния радиокнопки (проверена/не проверена)
    public bool Checked
    {
        get { return _Checked; }
        set
        {
            _Checked = value; // Установка состояния
            InvalidateControls(); // Обновление состояния других радиокнопок
            CheckedChanged?.DynamicInvoke(this); // Вызов события изменения состояния
            Invalidate(); // Обновление изображения
        }
    }
    ...
    protected override void OnMouseDown(System.Windows.Forms.MouseEventArgs e)
    {
        if (!_Checked) Checked = true; // Изменение состояния
        base.OnMouseDown(e); // Вызов базового метода
    }

    // Событие изменения состояния радиокнопки
    public event EventHandler CheckedChanged;

    // Обработка создания радиокнопки
    protected override void OnCreation()
    {
        InvalidateControls(); // Обновление состояния
    }

    // Обновление состояния других радиокнопок
    private void InvalidateControls()
    {
        ...
        {
            if (C != this && C is ChromeRadioButton)
            {
                ((ChromeRadioButton)C).Checked = false; // Сброс состояния других радиокнопок
            }
        }
    }
}
...
public class ChromeSeparator : ThemeControl154
{
    ...
    {
        LockHeight = 1; // Фиксированная высота
        BackColor = Color.FromArgb(238, 238, 238); // Установка фона
    }

    protected override void ColorHook()
    {
        // Переопределение метода без реализации
    }
...
{
    private Color _squareColor = Color.FromArgb(78, 87, 100);
    ...
    {
        base.CreateHandle();
        Alignment = TabAlignment.Left; // Установка выравнивания вкладок
    }

    // Свойство для цвета квадрата
    ...
    // Свойство для отображения внешних границ
    ...
    protected override void OnPaint(PaintEventArgs e)
    {
        ...
        using (var G = Graphics.FromImage(B))
        {
            try
            {
                SelectedTab.BackColor = Color.White; // Установка цвета для выбранной вкладки
            }
            catch
            {
            ...
            for (int i = 0; i < TabCount; i++)
            {
                ...
                var x2 = new Rectangle(tabRect.X - 2, tabRect.Y - 2, tabRect.Width + 3, tabRect.Height - 1);
                var textRectangle = new Rectangle(x2.X + 20, x2.Y, x2.Width - 20, x2.Height);

                if (i == SelectedIndex)
                {
                    G.FillRectangle(new SolidBrush(_squareColor), new Rectangle(x2.Location, new Size(9, x2.Height)));

                    DrawTabContent(G, i, textRectangle, Brushes.Black);
                }
                else
                {
                    DrawTabContent(G, i, textRectangle, Brushes.DimGray);
                }
            }

            e.Graphics.DrawImage(B, 0, 0); // Рисуем изображение на графике
        }
    }


    private void DrawTabContent(Graphics G, int index, Rectangle textRectangle, Brush textBrush)
    {
        if (ImageList != null)
        {
            try
            {
                ...
                {
                    G.DrawImage(ImageList.Images[TabPages[index].ImageIndex], new Point(textRectangle.X + 8, textRectangle.Y + 6));
                    G.DrawString("      " + TabPages[index].Text, Font, textBrush, textRectangle, new StringFormat { LineAlignment = StringAlignment.Center, Alignment = StringAlignment.Near });
                    return;
                }
            }
            catch
